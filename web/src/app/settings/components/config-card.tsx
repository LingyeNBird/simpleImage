...
export function ConfigCard() {
  ...
  const setImageBedCleanupDays = useSettingsStore((state) => state.setImageBedCleanupDays);
    ...
    <Card className="rounded-2xl border-white/80 bg-white/90 shadow-sm">
      <CardContent className="space-y-4 p-6">
        <div className="grid gap-4 md:grid-cols-2">
          ...
          <div className="space-y-2">
            <label className="text-sm text-stone-700">图床清理时效</label>
            <Input
              value={String(config?.image_bed_cleanup_days || "")}
              onChange={(event) => setImageBedCleanupDays(event.target.value)}
              placeholder="天数"
              className="h-10 rounded-xl border-stone-200 bg-white"
            />
            <p className="text-xs text-stone-500">超过该天数的本项目图床图片，会在上传新图时顺带清理。默认 3 天。</p>
          </div>
          <div className="space-y-2">
